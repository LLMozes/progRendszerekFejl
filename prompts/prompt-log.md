...
**Projekt:** Szokáskövető alkalmazás / Habit Tracker  
**Tárgy:** Programrendszerek fejlesztése  
**Félév:** 2026. tavasz  
**Fejlesztői eszközök:** ChatGPT, GitHub Copilot Chat  
**Cél:** A fejlesztés során használt fontosabb AI promptok dokumentálása és rövid értékelése.

---

## Tartalomjegyzék

1. [Projekttervezés – ChatGPT](#1-projekttervezés--chatgpt)
2. [Backend alap létrehozása – ChatGPT](#2-backend-alap-létrehozása--chatgpt)
3. [Prisma + SQLite adatmodell](#3-prisma--sqlite-adatmodell)
4. [Seed data](#4-seed-data)
5. [Session alapú autentikáció](#5-session-alapú-autentikáció)
6. [Habit CRUD API](#6-habit-crud-api)
7. [Habit completion API](#7-habit-completion-api)
8. [Admin API](#8-admin-api)
9. [Statisztika API](#9-statisztika-api)
10. [Frontend alap](#10-frontend-alap)
11. [Frontend autentikáció](#11-frontend-autentikáció)
12. [Habit CRUD frontend](#12-habit-crud-frontend)
13. [Habit szerkesztés javítása](#122-habit-szerkesztés-javítása)
14. [Habit completion frontend](#13-habit-completion-frontend)
15. [Dashboard statisztikák bekötése](#14-dashboard-statisztikák-bekötése)
16. [Admin frontend](#15-admin-frontend)
17. [Frontend UI/UX egységesítés](#16-frontend-uiux-egységesítés)
18. [Dokumentációs fájlok elkészítése](#17-dokumentációs-fájlok-elkészítése)
19. [AI promptelemzés elkészítése](#18-ai-promptelemzés-elkészítése)
20. [README véglegesítése](#19-readme-véglegesítése)
...
A prompt jól működött, mert a fejlesztés elején nem kódgenerálást kért, hanem egy átlátható megvalósítási tervet. Ez segített abban, hogy a projekt főbb mérföldkövei, technológiai rétegei és funkciói előre tisztázva legyenek.
...
A prompt jól működött, mert pontosan meghatározta a backend induló szerkezetét, de nem kérte még az adatbázis- vagy hitelesítési logika megvalósítását. Ez segített abban, hogy az alap Express szerver egyszerűen, jól elkülönítve készüljön el.
...
A prompt jól működött, mert egyértelműen megadta az adatmodellhez szükséges entitásokat és a projektstruktúra megtartását. A `User`, `Category`, `Habit`, `HabitCompletion` és `SystemLog` entitások lefedték a követelményben szereplő minimum öt entitásos elvárást.
...
A prompt jól működött, mert pontosan meghatározta a szükséges CRUD végpontokat és a jogosultsági szabályt, hogy a felhasználó csak a saját szokásait kezelheti.
...
## 14. Dashboard statisztikák bekötése
...
## 18. AI promptelemzés elkészítése

### Forrás

ChatGPT – az egész beszélgetés alatt történő chatelésből generált dokumentum.
...
A fejlesztés során használt promptok többsége jól működött, mert pontosan megadták az érintett technológiákat, fájlokat, végpontokat és korlátozásokat. Különösen hasznosnak bizonyultak azok a promptok, amelyek tartalmazták, hogy az AI mit ne módosítson, például backend kódot, dokumentációt vagy meglévő API útvonalakat.

A kevésbé jól működő eseteknél általában pontosítani kellett, hogy tényleges kódmódosításra van szükség, nem pedig prompt-log vagy dokumentáció frissítésére. A fejlesztés során ezért a promptok egyre részletesebbé és célzottabbá váltak.

Az AI használata támogatta a projekt elkészítését, de a generált kódot és dokumentációt fejlesztőként ellenőriztem, teszteltem és szükség esetén módosítottam.
